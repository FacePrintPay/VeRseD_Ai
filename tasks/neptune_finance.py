...
def communicate(message):
    ...
    bus.setdefault("Neptune", []).append(message)
    with open(BUS_FILE, "w") as b:
        json.dump(bus, b, indent=2)

def execute_task():
    now = datetime.datetime.now()
    entry = f"[{now}] Neptune Agent (CFO) - Duties: Track payments, optimize costs, MoonPay/Stripe logs\n"
