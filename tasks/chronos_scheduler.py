...
def communicate(message):
    ...
    bus.setdefault("Chronos", []).append(message)
    with open(BUS_FILE, "w") as b:
        json.dump(bus, b, indent=2)

def execute_task():
    now = datetime.datetime.now()
    entry = f"[{now}] Chronos Agent (Scheduler) - Duties: Timed tasks, cron replacements, reporting\n"
