...
def communicate(message):
    ...
    bus.setdefault("Callisto", []).append(message)
    with open(BUS_FILE, "w") as b:
        json.dump(bus, b, indent=2)

def execute_task():
    now = datetime.datetime.now()
    entry = f"[{now}] Callisto Agent (Reliability Engineer) - Duties: Process monitoring, watchdogs\n"
